...
func compositeTp(lhs FType, rhs FType) frt.Tuple2[FType, []UniRel] {
	switch _v316 := (lhs).(type) {
	case FType_FTypeVar:
		tv := _v316.Value
		switch _v317 := (rhs).(type) {
		case FType_FTypeVar:
			tv2 := _v317.Value
...
func lfdRetType(lfd LetFuncDef) FType {
	switch _v323 := (lfd.fvar.ftype).(type) {
	case FType_FFunc:
		ft := _v323.Value
		return freturn(ft)
	default:
