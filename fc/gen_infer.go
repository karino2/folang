...
func compositeTp(lhs FType, rhs FType) frt.Tuple2[FType, []UniRel] {
	switch _v306 := (lhs).(type) {
	case FType_FTypeVar:
		tv := _v306.Value
		switch _v307 := (rhs).(type) {
		case FType_FTypeVar:
			tv2 := _v307.Value
...
func lfdRetType(lfd LetFuncDef) FType {
	switch _v313 := (lfd.fvar.ftype).(type) {
	case FType_FFunc:
		ft := _v313.Value
		return freturn(ft)
	default:
