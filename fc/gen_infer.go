...
func compositeTp(lhs FType, rhs FType) frt.Tuple2[FType, []UniRel] {
	switch _v386 := (lhs).(type) {
	case FType_FTypeVar:
		tv := _v386.Value
		switch _v387 := (rhs).(type) {
		case FType_FTypeVar:
			tv2 := _v387.Value
...
func collectStmtRel(ec func(Expr) []UniRel, stmt Stmt) []UniRel {
	switch _v389 := (stmt).(type) {
	case Stmt_SExprStmt:
		se := _v389.Value
		return ec(se)
	case Stmt_SLetVarDef:
		slvd := _v389.Value
		switch _v390 := (slvd).(type) {
...
func lfdRetType(lfd LetFuncDef) FType {
	switch _v395 := (lfd.fvar.ftype).(type) {
	case FType_FFunc:
		ft := _v395.Value
		return freturn(ft)
	default:
...
func transVarStmt(transV func(Var) Var, transE func(Expr) Expr, stmt Stmt) Stmt {
	switch _v398 := (stmt).(type) {
	case Stmt_SLetVarDef:
		llvd := _v398.Value
		switch _v399 := (llvd).(type) {
		case LLetVarDef_LLOneVarDef:
			lvd := _v399.Value
