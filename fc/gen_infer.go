...
func compositeTp(lhs FType, rhs FType) frt.Tuple2[FType, []UniRel] {
	switch _v324 := (lhs).(type) {
	case FType_FTypeVar:
		tv := _v324.Value
		switch _v325 := (rhs).(type) {
		case FType_FTypeVar:
			tv2 := _v325.Value
...
func lfdRetType(lfd LetFuncDef) FType {
	switch _v331 := (lfd.fvar.ftype).(type) {
	case FType_FFunc:
		ft := _v331.Value
		return freturn(ft)
	default:
